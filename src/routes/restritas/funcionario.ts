...
export default async function funcionarioRoutes(fastify: FastifyInstance) {
    fastify.get('/funcionarios', { preHandler: [fastify.authenticate] }, async (_request, reply) => {
        const funcionarioRepository = AppDataSource.getRepository(Funcionario);
        try {
            const funcionarios = await funcionarioRepository.find({relations: ['pessoa']});
            return reply.status(201).send(funcionarios);
        } catch (error: any) {
            return reply.status(error.status || 400).send({ message: error.message || 'Erro ao buscar todos os funcionários do sistema', error });
        }
    });
