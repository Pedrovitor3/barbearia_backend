...
export class EmpresaController {
    static async listarFuncionariosDaEmpresa(
        ...
        const isAdmin = await adminRepo.findOneBy({ pessoaId: usuarioLogado.pessoaId });
        ...
        if (!podeListar) {
            throw { status: 403, message: 'Sem permissão para listar funcionários desta empresa.' };
        }

        // Busca todos os funcionários da empresa
        const funcionarios = await funcionarioRepo.find({ where: { empresaId }, relations: ['pessoa'] });
        return funcionarios;
    }
    ...
    static async criarEmpresa(
        ...
        if (!isAdmin) {
            throw { status: 403, message: 'Apenas administradores podem criar empresas.' };
        }
    ...
    static async listarEmpresasVinculadas(usuario: AuthenticatedUser): Promise<Empresa[]> {
        const adminRepo = AppDataSource.getRepository(Administrador);
        const funcionarioRepo = AppDataSource.getRepository(Funcionario);
        ...
        const funcionarios = await funcionarioRepo.find({ where: { pessoaId: usuario.pessoaId } });
        const empresasFuncionarioIds = funcionarios.map(f => f.empresaId);

        // Cliente: empresas onde é cliente
        const clientes = await clienteRepo.find({ where: { pessoaId: usuario.pessoaId } });
        const empresasClienteIds = clientes.map(c => c.empresaId);

        // Unir ids e buscar empresas
        const empresaIds = Array.from(new Set([...empresasFuncionarioIds, ...empresasClienteIds]));
    ...
    static async criarFuncionario(
        ...
        if (!podeCriar) {
            throw { status: 403, message: 'Sem permissão para criar funcionário nesta empresa.' };
        }
        ...
        let usuario: any = await usuarioRepo.findOneBy({ pessoaId: pessoa.pessoaId });
        if (!usuario) {
        ...
        if (funcionario) {
            throw { status: 400, message: 'Funcionário já existe para esta empresa.' };
        }
    ...
    static async editarFuncionario(
        ...
        if (!podeEditar) {
            throw { status: 403, message: 'Sem permissão para editar funcionário nesta empresa.' };
        }
